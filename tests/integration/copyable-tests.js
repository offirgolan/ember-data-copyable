...
export default function generateTests(options = { async: false }) {
  test('it shallow copies attributes', async function (assert) {
    ...
    let model = await getRecord(this, options, 'foo', 1);
  ...
  test('it shallow copies relationships', async function (assert) {
    ...
    let model = await getRecord(this, options, 'baz', 1);

    let copy = await model.copy(false);

    if (options.async) {
      await Promise.all([model.get('foos'), model.get('bar')]);
    }

    assert.equal(await model.get('bar.id'), await copy.get('bar.id'));
    assert.deepEqual(
  ...
  test('it copies belongsTo relationship', async function (assert) {
    ...
    let model = await getRecord(this, options, 'bar', 1, { reload: true });
  ...
  test('it copies belongsTo relationship by reference', async function (assert) {
    ...
    let model = await getRecord(this, options, 'bar', 1);
    ...
    assert.equal(
      await model.get('foo.property'),
      await copy.get('foo.property')
    );
    assert.notEqual(copy.get('id'), 1);
    assert.ok(await copy.get('foo.id'));
  });

  test('it copies empty belongsTo relationship', async function (assert) {
    assert.expect(3);

    let model = await getRecord(this, options, 'fooEmpty', 1);
    ...
    if (options.async) {
      assert.notOk(await copy.get('foo.id'));
    } else {
  ...
  test('it copies complex objects', async function (assert) {
    ...
    let model = await getRecord(this, options, 'multi', 1);
  ...
  test('it copies empty objects', async function (assert) {
    ...
    let model = await getRecord(this, options, 'multi', 2);
  ...
  test('it copies cyclical relationship', async function (assert) {
    ...
    let model = await getRecord(this, options, 'fooCycle', 1);
